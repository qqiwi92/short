export async function GET() {
  return new Response(
    JSON.stringify([
      {
        date: "16 минут назад",
        link: "https://habr.com/ru/news/819179/",
        title:
          "В ЛЭТИ создали прототип резервуарного компьютера для аппаратных нейроморфных вычислений на принципах магноники",
      },
      {
        date: "31 минуту назад",
        link: "https://habr.com/ru/news/819171/",
        title: "Репозиторий браузера Supermium на GitHub больше недоступен",
      },
      {
        date: "34 минуты назад",
        link: "https://habr.com/ru/companies/jetinfosystems/news/819167/",
        title: "Топ-5 ИБ-событий недели по версии Jet CSIRT",
      },
      {
        date: "3 часа назад",
        link: "https://habr.com/ru/news/819107/",
        title:
          "VK сообщила об изменениях в управлении «Одноклассниками», «VK Музыкой» и ML‑системами",
      },
      {
        date: "3 часа назад",
        link: "https://habr.com/ru/news/819105/",
        title: "«Яндекс» разрабатывает единую нейросеть для речи и текста",
      },
      {
        date: "3 часа назад",
        link: "https://habr.com/ru/news/819103/",
        title:
          "Производитель «Рикор» запустит производство собственных корпусов для ПК к концу 2024 года",
      },
      {
        date: "3 часа назад",
        link: "https://habr.com/ru/news/819099/",
        title:
          "Обновление Telegram: сообщения с анимированными эффектами, поиск по хештегам и подписи над файлами",
      },
      {
        date: "3 часа назад",
        link: "https://habr.com/ru/news/819093/",
        title:
          "Минкульт РФ переведёт до конца 2024 года центральный аппарат на ОС Astra Linux и систему управления доменом ALD Pro",
      },
      {
        date: "3 часа назад",
        link: "https://habr.com/ru/news/819091/",
        title: "Некоторые госсайты недоступны из-за сбоя",
      },
      {
        date: "3 часа назад",
        link: "https://habr.com/ru/news/819089/",
        title: "Банки оценили перспективы единой биометрической площадки НСПК",
      },
      {
        date: "4 часа назад",
        link: "https://habr.com/ru/companies/mws/news/819083/",
        title:
          "Рассказывает MWS: как использовать ресурсы для работы с графикой",
      },
      {
        date: "4 часа назад",
        link: "https://habr.com/ru/news/819081/",
        title: "Docker Hub разблокировали по геоайпи в России",
      },
      {
        date: "4 часа назад",
        link: "https://habr.com/ru/companies/1cupis/news/819077/",
        title:
          "Приглашаем на митап в Москве «Инструменты обеспечения безопасной разработки»",
      },
      {
        date: "4 часа назад",
        link: "https://habr.com/ru/news/819075/",
        title:
          "Microsoft объявила об инвестиции $3,2 млрд в инфраструктуру облачных вычислений и искусственного интеллекта Швеции",
      },
      {
        date: "4 часа назад",
        link: "https://habr.com/ru/news/819073/",
        title:
          "«Бауманка» заявила о выпуске на рынок модуля считывания для сверхпроводниковых квантовых компьютеров",
      },
      {
        date: "5 часов назад",
        link: "https://habr.com/ru/companies/sberbank/news/819057/",
        title:
          "Для пользователей GitVerse стало доступно российское «зеркало» Docker Hub",
      },
      {
        date: "5 часов назад",
        link: "https://habr.com/ru/news/819049/",
        title:
          "«Додо Пицца» подала в суд на Telegram, чтобы получить ник @dodopizza",
      },
      {
        date: "5 часов назад",
        link: "https://habr.com/ru/news/819047/",
        title:
          "ИИ-функции в iOS 18 будут поддерживать только iPhone 15 Pro и Pro Max",
      },
      {
        date: "5 часов назад",
        link: "https://habr.com/ru/companies/ruvds/news/819017/",
        title:
          "Лето на все 100! Скидка 30%, iPad Pro 13 и другие подарки для клиентов RUVDS",
      },
      {
        date: "6 часов назад",
        link: "https://habr.com/ru/news/819039/",
        title: "Сеть продуктовых магазинов «Верный» атаковали хакеры",
      },
    ])
  );
}
